...
class CardMyAuction extends Component<IProps> {
    ...
    async payAucSubmit(values: any){
        let amount = web3.utils.toWei(values.amount.toString(), 'ether');
        let id = this.props.product.aucid;
        try {
            let ret = await contract.methods.payNFT(id).send({
                from: this.state.address,
                value: amount
            })
            message.success('成功付款!');
            window.location.href = '/home';
        }
        catch(e){
            message.error('付款失败，请检查！');
        }
    }
    ...
    renderContent = (column = 2) => (
        <Descriptions size="small" column={column}>
            ...
            <Descriptions.Item label="起拍价">
                {this.props.product.startPrice}
            </Descriptions.Item>
            <Descriptions.Item label="当前最高出价">
                {this.props.product.highestBid}
            </Descriptions.Item>
        </Descriptions>
    );
    render() {
